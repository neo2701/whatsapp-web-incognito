...
browser.runtime.onMessage.addListener(function (messageEvent, sender, callback) {
  if (messageEvent.name == "setOptions") {
    ...
    if ("saveEditedMsgs" in messageEvent) {
      chrome.storage.local.set({ saveEditedMsgs: messageEvent.saveEditedMsgs });
    }
    ...
    var saveEditedMsgs = false;
    var showDeviceTypes = true;
    var autoReceiptOnReplay = true;

    chrome.storage.local.get(["presenceUpdatesHook", "readConfirmationsHook", "showReadWarning", "safetyDelay", "saveDeletedMsgs", "showDeviceTypes", "autoReceiptOnReplay", "saveEditedMsgs"]).then(function (storage) {
      ...
      if (storage["saveEditedMsgs"] != undefined) {
        saveEditedMsgs = storage["saveEditedMsgs"];
      }
