  ...
  if (firstMenuItem != undefined) {
    ...
    browser.runtime.sendMessage({ name: "getOptions" }, function (options) {
      ...
      drop.on("open", function () {
        ...
        document.getElementById("incognito-option-save-edited-msgs").addEventListener("click", onSaveEditedMsgsTick);
...
function generateDropContent(options) {
  var presenceCaption = isMultiDevice ? "Will prevent you from seeing other people's last seen" : "Blocks outgoing presence updates.";
  var deletedMessagesTitle = "Restore deleted messages";
  var deletedMessagesCaption = isMultiDevice ? "Marks deleted messages in red" : "Saves deleted messages and restores them later.";

  var editedMessagesTitle = "Save version of edited messages";
  var editedMessagesCaption = "Saves the original version of edited messages and restores them later.";
  ...
  var presenceUpdatesChcekbox = options.presenceUpdatesHook
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : "unchecked " + "'> <div class='checkmark incognito-mark" + "'> </div>";
  var saveDeletedMessagesCheckbox = options.saveDeletedMsgs
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : "unchecked " + "'> <div class='checkmark incognito-mark" + "'> </div>";
  var saveEditedMessagesCheckbox = options.saveEditedMsgs
    ? "checked incognito-checked'> \
        <div class='checkmark incognito-mark incognito-marked'> </div>"
    : "unchecked " + "'> <div class='checkmark incognito-mark" + "'> </div>";
            ...
            <div class='incognito-options-navigator'>
                ...
                <div class='incognito-options-view' id='incognito-options-view2'>
                    <div class='incognito-options-view' id='incognito-options-view1'>
                        ...
                        <div id='incognito-option-save-edited-msgs' class='incognito-options-item' style='cursor: pointer;'>
                        <div class='checkbox-container-incognito' style=''>
                            <div class='checkbox checkbox checkbox-incognito ${saveEditedMessagesCheckbox}
                            </div>
                        </div>
                        ${editedMessagesTitle}
                        <div class='incognito-options-description'>${editedMessagesCaption}</div>
                    </div>
...
function getTheme() {
  ...
  if (localStorage["theme"] != "null" && localStorage["theme"] != undefined) return localStorage["theme"];
    ...
    if ((window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches) || document.getElementsByClassName("web")[0].classList.contains("dark")) return '"dark"';
    else return '"light"';
  }
}
...
function onSaveEditedMsgsTick() {
  var saveEditedMsgsHook = false;
  var checkbox = document.querySelector("#incognito-option-save-edited-msgs .checkbox-incognito");
  var checkboxClass = checkbox.getAttribute("class");
  var checkmark = checkbox.firstElementChild;
  var chekmarkClass = checkmark.getAttribute("class");
  if (checkboxClass.indexOf("unchecked") > -1) {
    tickCheckbox(checkbox, checkmark);
    saveEditedMsgsHook = true;
  } else {
    untickCheckbox(checkbox, checkmark);
    saveEditedMsgsHook = false;
  }
  browser.runtime.sendMessage({ name: "setOptions", saveEditedMsgs: saveEditedMsgsHook });
  document.dispatchEvent(
    new CustomEvent("onOptionsUpdate", {
      detail: JSON.stringify({ saveEditedMsgs: saveEditedMsgsHook }),
    })
  );
}
