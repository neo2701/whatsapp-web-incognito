...
var saveEditedMsgsHookEnabled = true;
...
initialize();
let ProtocolMessage = Message.ProtocolMessage;
      ...
      if (data instanceof ArrayBuffer || data instanceof Uint8Array) {
        ...
        for (var i = 0; i < decryptedFrames.length; i++) {
          ...
          if (isInitializing) {
            ...
            document.dispatchEvent(new CustomEvent("onInterceptionWorking", { detail: JSON.stringify({ isInterceptionWorking: true, isMultiDevice: isMultiDevice }) }));
          }
...
NodeHandler.isSentNodeAllowed = function (node, tag) {
  ...
  for (var i = 0; i < subNodes.length; i++) {
    ...
    var shouldBlock = (readConfirmationsHookEnabled && action === "read") || (readConfirmationsHookEnabled && action == "receipt" && data["type"] == "read") || (readConfirmationsHookEnabled && action == "receipt" && data["type"] == "read-self") || (readConfirmationsHookEnabled && action == "receipt" && data["type"] === "played") || (readConfirmationsHookEnabled && action == "received" && data["type"] === "played") || (presenceUpdatesHookEnabled && action === "presence" && data["type"] === "available") || (presenceUpdatesHookEnabled && action == "presence" && data["type"] == "composing") || (presenceUpdatesHookEnabled && action == "chatstate" && child.content[0].tag == "composing");
...
NodeHandler.onMessageNodeReceived = async function (currentNode, message, isMultiDevice, encNodes, messageNodes) {
  ...
  var isEditedMessage = NodeHandler.checkForEditedMessageNode(message, messageId, remoteJid);
...
NodeHandler.checkForMessageDeletionNode = function (message, messageId, remoteJid) {
  ...
  //
  // console.log(ProtocolMessage.Type);
  var messageRevokeValue = ProtocolMessage.Type.REVOKE.value;
...
function onEditedMessageBlocked(message, remoteJid, messageId, editedMessageId) {
  console.log("WhatsIncognito: --- Blocking message EDIT action! ---");
  console.log(message, remoteJid, messageId, editedMessageId, "edit");
}
...
async function saveDeletedMessage(retrievedMsg, deletedMessageKey, revokeMessageID) {
  ...
  if (retrievedMsg.isMedia || retrievedMsg.mediaKey) {
    ...
    try {
      const decryptedData = await WhatsAppAPI.downloadManager.downloadAndMaybeDecrypt({ directPath: retrievedMsg.directPath, encFilehash: retrievedMsg.encFilehash, filehash: retrievedMsg.filehash, mediaKey: retrievedMsg.mediaKey, type: retrievedMsg.type, signal: new AbortController().signal });
