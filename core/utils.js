...
function nodeToXML(node) {
  if (typeof node == ArrayBuffer) return "<ArrayBuffer (" + node.byteLength + " bytes)>";
...
function getCurrentChat() {
  if (window.WhatsAppAPI && WhatsAppAPI.Store && WhatsAppAPI.Store.Chat && WhatsAppAPI.Store.Chat.getActive) return WhatsAppAPI.Store.Chat.getActive();
...
function isChatBlocked(jid) {
  ...
  for (jid in blockedChats) {
    if (jid.split("@")[0].split(":")[0] == user) return true;
  }
...
function showToast(message) {
  ...
  Velocity(toast, { scale: [1, 0], opacity: [1, 0] }, { defaultDuration: 300, easing: [0.1, 0.82, 0.25, 1] });
  setTimeout(function () {
    Velocity(toast, { scale: [0, 1], opacity: [0, 1] }, { defaultDuration: 300, easing: [0.1, 0.82, 0.25, 1] });
  }, 4000);
}
...
function getCSSRule(ruleName) {
  ...
  for (var i = 0; i < styleSheets.length; ++i) {
    ...
    for (var j = 0; j < styleSheetRules.length; ++j) rules[styleSheetRules[j].selectorText] = styleSheetRules[j];
  }
...
function makeLogMessage(args) {
  function mergeArrays(e, t) {
    const n = [],
      ...
      i = t.length,
      a = Math.max(r, i);
    for (let o = 0; o < a; o++) o < r && n.push(e[o]), o < i && n.push(t[o]);
    return n;
  }

  for (var l = new Array(args.length > 1 ? args.length - 1 : 0), d = 1; d < args.length; d++) l[d - 1] = args[d];
